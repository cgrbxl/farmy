...
$('#test-bars').innerHTML = [{n:19,cls:'foundation'}, ...FARMY.slices.map((s,i) => ({n:s.tests,cls:`uc${i+1}`}))].map(item => `<span class="${item.cls}" style="flex:${item.n}">${item.n}</span>`).join('');
...
$('#next-queue').innerHTML = FARMY.queue.map((item,index) => `<article><span class="queue-number">${String(index+4).padStart(2,'0')}</span><div><span class="tag planned">${index === 0 ? 'Next queued outcome' : 'Queued'}</span><h4>${item.title}</h4><p>${item.detail}</p></div></article>`).join('');
