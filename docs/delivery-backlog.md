...
Status: proposed order, not a schedule or implementation report. UC-001 through UC-006 have runnable synthetic macOS demonstrations and acceptance suites; later slices remain unimplemented. Follow the [working method](working-method.md); only one implementation slice should be active at a time.
