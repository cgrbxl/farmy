...
Status: proposed order, not a schedule or implementation report. UC-001, UC-002, UC-003 and UC-004 have runnable synthetic macOS demonstrations and acceptance suites; later slices remain unimplemented. Follow the [working method](working-method.md); only one implementation slice should be active at a time.
