...
| Exchange | Prepare an immutable disclosure manifest and preview, then deliver exact authorised bytes to the named recipient. UC-006 delivers to a separate loopback recipient fixture; production recipient adapters remain future work. Track attempt IDs, checksums and acknowledgement uncertainty. | Separate export permission; preview/approval bound to content, recipient and purpose; no blind duplicate delivery after an ambiguous result |
